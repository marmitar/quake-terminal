 ...
 * @property {(outdir: string, result: esbuild.BuildResult) => Promise<esbuild.OnEndResult>} run Plugin body.
 */
  ...
  return {
    ...
    setup(build) {
      ...
      build.onEnd(async (result) => {
        try {
          return await run(outdir, result);
        } catch (error) {
          return {
...
await esbuild.build({
  ...
  plugins: [
    ...
    onEnd({
      ...
      run: async (outdir, result) => {
        if (result.errors.length > 0) {
          return {
            warnings: [
              {
                text: "'gnome-extensions pack' skipped because of previous errors",
              },
            ],
          };
        }
